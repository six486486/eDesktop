...
点击聊天窗口的小铃铛，打开“小栖设置”，可以调整天气城市、播报时间、“我的一天”提前多久提醒，以及是否带一声短短的“喵”。天气播报也能直接用一句话修改。
...
不喜欢回复太长，或者不想每句话后面都跟着一个问题？可以直接告诉小栖：“以后叫我小李，回答短一点，不要追问我。”

也可以点 **小铃铛 → 聊天偏好**，自己修改用户称呼、回复长短和追问方式。两种入口改的是同一份偏好，以最后一次明确修改为准；手动设置不用启动 Ollama，保存后从下一条回复生效。称呼留空，就不会特意叫你的名字。

![聊天偏好：设置称呼、回复长短和追问方式](docs/screenshots/pet-preferences.png)

偶尔想多听一点，说“这次详细一点”就好，不会把平时的偏好一起改掉。想重新设置，可以点“恢复默认”，再点“保存偏好”。
...
| `npm run test:pet-chat-preferences` | 聊天偏好的保存、对话同步与界面检查 |
...
小栖把模型理解和程序规则放在一起用。“以后叫我小李”这类明确的聊天偏好先按规则处理；提醒、天气和番茄钟操作主要由本地 LLM 判断，再交给程序检查参数、计算时间和执行。保存成功后才给出真实回执，到点提醒也由程序负责。

每轮请求都有独立标识，新消息可以打断旧回复，迟到结果不能覆盖新状态。记忆保留原话来源，操作记录和业务数据一起保存。目前这些有数量上限的数据存成 JSON 就够用，还没有引入 SQLite、向量检索或自主多步执行；具体取舍写在[桌宠实现说明](desktop-pet/README.md#当前技术选择)里。
