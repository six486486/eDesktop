...
app.whenReady().then(async () => {
  ...
  chat.destroy()
  await board('pet-chat', 960, 822, [{ ...chatImage, x: 40, y: 46, framed: true }, { ...settingsImage, x: 500, y: 46, framed: true }])
  await board('pet-memory', 960, 822, [{ ...memoryImage, x: 40, y: 46, framed: true }, { ...operationImage, x: 500, y: 46, framed: true }])
  const preferences = await petView('chat', { ...petBase, chatPreferences: { preferredName: '小李', replyLength: 'short', followUp: 'avoid' } }, 370, 490)
  await click(preferences, '小栖设置'); await click(preferences, '聊天偏好')
  await ready(preferences, '.chat-preferences')
  const preferencesImage = await capture(preferences, 'preferences', false)
  preferences.destroy()
  await board('pet-preferences', 450, 570, [{ ...preferencesImage, x: 40, y: 40, framed: true }])
